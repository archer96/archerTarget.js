 ...
 * @param {Object|Array}  arrow.style       Style of the arrow(s) or arrowset(s). Construction should look like defaultParams.style.initial.
 *                                          If arrow.style is an array, the order has to be the same as arrow.arrowsetID or arrow.arrowID.
 */
jat.Target.prototype.setArrowStyle = function (arrow) {
      
    var self = this,
        i,
        j,
        defaultConfig = {
            style: this.params.arrowDefaults.style.initial
        },
        /**
        ...
        setArrowset = function (arrowSetID, style) {
            ...
            if (!style.radius) {
                style.radius = self.arrow[arrowSetID].radius;
            }

            for (j = 0; j < self.arrow[arrowSetID].data.length; j++) {
        
                arrowObj = self.arrow[arrowSetID].data[j];

                arrowObj.element.setStyle({
                    stroke: style.stroke,
                    radius: style.radius,
                    fill: style.color
                });

                $(arrowObj.element).css({
        ...
        setArrow = function (arrowSetID, arrowID, style) {
            ...
            var arrowObj = self.arrow[arrowSetID].data[arrowID];
    ...
    /*
     * If no arrow id is given and arrow.arrowsetID is not an array, we only set the style of one arrowset
     */
    } else {
