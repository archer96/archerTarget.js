jat.Target.prototype.createArrows =  function (arrows) {
    ...
    for (i = 0; i < arrowLength; i++) {
        ...
        arrow = arrow instanceof Array ? { data: arrow} : arrow;
                
        arrow = $.extend(true, {}, self.params.arrowDefaults, arrow);
        ...
        for (j = 0; j < dataLength; j++) {
            ...
            arrowData.element = self.canvas.createCircle({
                x: self.convertTo.px.x(arrowData.x, arrowData.target),
                y: self.convertTo.px.y(arrowData.y, arrowData.target),
                radius: arrow.radius,
