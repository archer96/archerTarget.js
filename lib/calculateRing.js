jat.Target.prototype.calculateRing =  function (config) {
    ...
    var i,
        ...
        targetTemp = {
            x: (this.convertTo.canvasX(target.center[0]) + this.transX) * this.zoom,
            y: (this.convertTo.canvasY(target.center[1]) + this.transY) * this.zoom,
            radius: this.convertTo.canvasX(target.diameter) / 2 * this.zoom,
    ...
    if (distanceToCenter.diagonal <= targetTemp.radius + 1) {
        ...
        for (i = targetTemp.numberRings - 1; i >= 0; i--) {
            ...
            if (distanceToCenter.diagonal <= this.convertTo.canvasX(currentTarget.diameters[i], target.diameter) / 2 * this.zoom + 1) {
