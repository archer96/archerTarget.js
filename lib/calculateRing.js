jat.Target.prototype.calculateRing =  function (config) {
    ...
    var i,
        ...
        targetTemp = {
            x: this.convertTo.canvas.x(this.target[config.target].center[0]),
            y: this.convertTo.canvas.y(this.target[config.target].center[1]),
            radius: this.convertTo.canvas.x(this.target[config.target].diameter) / 2,
    ...
    if (distanceToCenter.diagonal <= targetTemp.radius + 1) {
        ...
        for (i = targetTemp.numberRings - 1; i >= 0; i = i - 1) {
                    
            if (distanceToCenter.diagonal <= this.convertTo.canvas.x(currentTarget.diameters[i], this.target[config.target].diameter) / 2 + 1) {
