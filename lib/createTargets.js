jat.Target.prototype.createTarget = function (targets) {
    ...
    for (i = 0; i < len; i++) {
        ...
        target.element = this.canvas.createGroup(false, { id: $(this.container).attr('id') + 'Target_' + i, eleClass: 'targetCanvas' });
        ...
        for (j = 0; j < jat.Target.targets[target.name].numberRings; j++) {
            ...
            target.rings[j].element = this.canvas.createCircle({
                x: this.convertTo.canvas.x(target.center[0]),
                y: this.convertTo.canvas.y(target.center[1]),
                radius: this.convertTo.canvas.x(jat.Target.targets[target.name].diameters[j], target.diameter) / 2,
