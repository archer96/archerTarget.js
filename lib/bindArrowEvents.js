jat.Target.prototype.bindArrowEvents = function () {
    ...
    var move = function () {
            ...
            if (self.arrowMoving && arrowsetTmp.data.draggable) {
                ...
                arrowTmp.data.x = self.convertTo.pc.x(curPageX, arrowTarget);
                arrowTmp.data.y = self.convertTo.pc.y(curPageY - pointerHeight, arrowTarget);
            ...
            if (e.type === 'mousedown') {
                ...
                if (arrowsetTmp.data.draggable instanceof Object && arrowsetTmp.data.draggable) {
                    ...
                    self.createArrowPointer({
                        x: self.convertTo.px.x(arrowTmp.data.x, arrowTarget),
                        y: self.convertTo.px.y(arrowTmp.data.y, arrowTarget),
                        drag: arrowsetTmp.data.draggable,
    ...
    this.container.on('mousemove', onMouseMove)
        .find('.arrowSetCanvas').on('mousedown mouseover', 'circle', onMouseDown)
        .on('mouseup mouseout click', 'circle', onMouseUp);
